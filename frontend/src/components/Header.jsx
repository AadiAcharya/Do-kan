      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-5">
            <span className="text-sm text-gray-500 hidden sm:block">Help & Support</span>
            <button onClick={() => navigate("/cart")} className="relative text-sm font-medium hover:text-blue-600">
              🛒 Cart
              {itemCount > 0 && (
                <span className="absolute -top-2 -right-3 bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{itemCount}</span>
              )}
            </button>
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                ...
                {user?.role === "admin" && (
                  <button onClick={() => navigate("/admin")} className="text-sm font-medium text-blue-600 hover:underline">Admin</button>
                )}
                <button onClick={handleLogout} className="text-sm font-medium text-red-500 hover:underline">Logout</button>
              </div>
            ) : (
              <button onClick={() => navigate("/login")} className="bg-blue-600 text-white text-sm px-4 py-1.5 rounded-lg hover:bg-blue-700 transition">Sign In</button>
            )}
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => navigate("/cart")} className="text-blue-600 text-xl">🛒</button>
        </div>
      </div>
    </header>
  );
